...
do
	...
	do
     ...
     ./bin/awl_OP_focused_reading -seed ${i} -trained_weights Oral_Weight_${k}_v${i} -epsilon 0.05 -iteration 1000

     echo "OS focused training: version=${i}, proficiency level=${k}"
     ./bin/awl_OS_focused_reading -seed ${i} -trained_weights Oral_Weight_${k}_v${i} -epsilon 0.05 -iteration 1000
