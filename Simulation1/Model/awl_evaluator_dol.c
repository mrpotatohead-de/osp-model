...
int main(int argc,char *argv[])
{
  ...
  int con=0;
  float sse_phon=0;
  int OPalone=0, SPalone=0, PPalone=0, OSalone=0, PSalone=0, CSalone=0, SSalone=0;
  ...
  for(i=1;i<argc;i++)
	...
	#endif
     else if (strncmp(argv[i],"-OPalone",5)==0)
	{
	  OPalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-SPalone",5)==0)
	{
	  SPalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-PPalone",5)==0)
	{
	  PPalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-OSalone",5)==0)
	{
	  OSalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-PSalone",5)==0)
	{
	  PSalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-CSalone",5)==0)
	{
	  CSalone=atof(argv[i+1]);
	  i++;
	}
	else if (strncmp(argv[i],"-SSalone",5)==0)
	{
	  SSalone=atof(argv[i+1]);
	  i++;
	}
      else
	{
...
//printf("Samples: %d\n", SAMPLES);
  ...
  if (weightFile==NULL) Error0("no weight file specified");

  if(strcmp(patternFile,"ps_randcon_awl.pat")==0 && testreadingmodel==0) load_weights(ps,weightFile);
  if(strcmp(patternFile,"ps_randcon_awl.pat")==0 && testreadingmodel==1) load_weights(reading,weightFile);
  if( strcmp(patternFile,"sp_awl.pat")==0 && testreadingmodel==0) load_weights(sp,weightFile);
  if( strcmp(patternFile,"sp_awl.pat")==0 && testreadingmodel==1) load_weights(reading,weightFile);

 if( strcmp(patternFile,"englishdict_randcon_awl.pat")==0) {
  //printf("englishdict_randcon_awl: load weights\r\n");
  ...
  for(i=0;i<examples->numExamples;i++)
    {
      ...
      if( strcmp(patternFile,"ps_randcon_awl.pat")==0 && testreadingmodel==0){printf("ps test "); crbp_forward(ps,ex);}
      if( strcmp(patternFile,"ps_randcon_awl.pat")==0 && testreadingmodel==1){printf("ps test "); crbp_forward(reading,ex);}
      if( strcmp(patternFile,"sp_awl.pat")==0 && testreadingmodel==0){printf("sp test ");  crbp_forward(sp,ex);}
      if( strcmp(patternFile,"sp_awl.pat")==0 && testreadingmodel==1){printf("sp test ");  crbp_forward(reading,ex);}
     

     if( strcmp(patternFile,"englishdict_randcon_awl.pat")==0 ){printf("reading test ");  crbp_forward(reading,ex);}
      push_item(error,stats);
      euclid_wrong=0;
      get_name(ex->name,name);
      printf("%s\t",name);
      if( (strcmp(patternFile,"sp_awl.pat")==0  && !con) || (strcmp(patternFile,"englishdict_randcon_awl.pat")==0 && !semantic)){
	  ...
	  error=nonword_error(phonology->outputs[SAMPLES-1],words_pho[i],words_pho2[i]);
          ...
          if(strcmp(words_pho[i],euclid_output)==0) {
              printf("correct\t");
              correct=1;
          }
          else {
       ...
       if ( strcmp(patternFile,"ps_randcon_awl.pat")==0 || (strcmp(patternFile,"englishdict_randcon_awl.pat")==0 && semantic)){
