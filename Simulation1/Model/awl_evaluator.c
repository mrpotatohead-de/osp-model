...
//printf("Samples: %d\n", SAMPLES);
 ...
 if( strcmp(patternFile,"englishdict_randcon_awl_neworth.pat")==0) {
  ...
  for(i=0;i<examples->numExamples;i++)
    {
     ...
     if( strcmp(patternFile,"englishdict_randcon_awl_neworth.pat")==0 ){printf("reading test ");  crbp_forward(reading,ex);}
      push_item(error,stats);
      euclid_wrong=0;
      get_name(ex->name,name);
      printf("%s\t",name);
      if( (strcmp(patternFile,"sp_awl.pat")==0  && !con) || (strcmp(patternFile,"englishdict_randcon_awl_neworth.pat")==0 && !semantic)){
	  ...
	  error=nonword_error(phonology->outputs[SAMPLES-1],words_pho[i],words_pho2[i]);
       ...
       if ( strcmp(patternFile,"ps_randcon_awl.pat")==0 || (strcmp(patternFile,"englishdict_randcon_awl_neworth.pat")==0 && semantic)){
